...
def load_llm_model() -> Tuple[FastLanguageModel, AutoTokenizer]:
    ...
    model_id = "woshityj/gemma_2_2B_Instruct_bnb_finetuned"
...
def evaluate_fact_checking_pipeline(llm_model, tokenizer, question_duplicate_model, question_duplicate_tokenizer, passage_ranker):
    ...
    for claim in check_claims_json[850:900]:
    ...
    updated_data = [
        ...
        for item, bool_value in zip(check_claims_json[850:900], factual_labels)
    ]

    with open("../../fact_checking_evaluation_data/claims_with_labels_gemma_finetuned.jsonl", "a") as f:
...
def evaluate_normal(llm_model, tokenizer):
    ...
    with open("../../fact_checking_evaluation_data/claims_with_labels_gemma.jsonl", "w") as f:
        for item in updated_data:
            f.write(json.dumps(item) + "\n")
